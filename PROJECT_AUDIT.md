...
| 2026-05-08 | Sprint 1: SEC-H5 | Admin-проверка в Telegram-хендлерах вынесена из 18 ручных `if not _is_admin_tg` в router-level `IsAdminFilter` (`app/filters/admin.py`). На degraded path (AppSettings unavailable) — `logger.exception` + fallback на env. | (этот коммит) |
