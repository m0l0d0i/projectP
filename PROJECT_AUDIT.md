...
| 2026-05-09 | Sprint 2: FEA-NOTIF новые jobs + расширение клавиатуры | Миграция `20260509_000026_subscription_trial_notify_flags` (3 idempotency-флага на `subscriptions`). Jobs `check_trial_milestones` (1ч интервал, mid/last_day/post_expire с idempotency через колонки) и `check_weekly_usage_report` (24ч, dedup через Redis-cooldown 6д из правила) в `app/services/notifications.py` + регистрация в `build_scheduler`. `low_traffic_alert_keyboard` расширена до «➕ 50 ГБ / ➕ 100 ГБ» (через `TopUpCallback`), «⏳ Продлить 1 мес» (renew_early), «🔕 Не напоминать 24ч» (новый `NotificationCallback(action=snooze, code=...)`). `NotificationDispatcher.snooze_key()` + проверка `_is_snoozed` перед cooldown'ом + метрика `BLOCKED{reason=snoozed}`. Snooze-handler в `vpn.py` пишет Redis-ключ `notif_snooze:{user_id}:{code}` на 24ч. Repository: `SubscriptionRepository.trial_pending_milestones(since)`. | (этот коммит) |
| 2026-05-09 | Sprint 2: FEA-NOTIF метрики Prometheus | `vpn_bot_notifications_sent_total{code,status}` (status: `ok`/`fallback`) и `vpn_bot_notifications_blocked_total{code,reason}` (reason: `disabled`/`cooldown`/`template_error`) добавлены в `app/observability/metrics.py`; `NotificationDispatcher` инкрементит соответствующие counter'ы (template_error → blocked + sent с `status=fallback`, успех → sent c `status=ok`, disabled/cooldown → blocked + return False). | (этот коммит) |
| 2026-05-10 | Sprint 2: FEA-A9 закрыт + Sprint 2 закрыт целиком | FEA-A9 (mid-cycle апсейл устройств) реализован тремя коммитами. **Backend** (commit `50fd225`): миграция 27→28 (`InvoicePurpose.device_topup` enum + 3 поля в `app_settings`: `mid_cycle_device_topup_enabled` / `_price_mode` (prorated|fixed) / `_fixed_price` 99 ₽ default + 2 CHECK). `PricingService.quote_device_topup(...)` → `DeviceTopupQuote` (prorated = `device_step_price × days_left/days_in_cycle` с округлением вверх до рубля как в `calculate_reset_price`; fixed = `_fixed_price` без proration). `PaymentService.create_device_topup_invoice` + `quote_device_topup` (последний используется как preview без создания «висячего» invoice). `SubscriptionService._apply_device_topup_invoice` (инкремент `used_device_count`, single→custom, `MarzbanClient.set_online_limit` — no-op если `marzban_online_limit_field` не сконфигурирован). **UX** (commit `23f70ff`): кнопка «➕ Добавить устройство» в `vpn_details_keyboard` с условиями `_can_add_device` (не trial, не unlimited-traffic, expire_date в будущем, mode!=unlimited, `count+1 ≤ MAX_CUSTOM_DEVICES`, `mid_cycle_device_topup_enabled=True`). Preview-экран в `purchase.py` (`VpnCallback(action=add_device)` через `service.quote_device_topup`) + invoice-flow на confirm (`add_device_confirm`). Расширены `render_invoice_text` и success-message в `confirm_invoice` для `device_topup`. **Admin**: `/admin/upsells/devices/` — миграция 29 (`audit_action += mid_cycle_device_settings_updated`), GET+POST в `routes.py`, шаблон `admin_upsells_devices.html` с on/off + radio prorated|fixed + фикс-ценой + live-таблицей превью на 30/15/5/1 день. Sprint 2 (FEA-NOTIF + FEA-A8 + FEA-A9) закрыт целиком. | (этот коммит) |
| 2026-05-09 | Sprint 2: FEA-A8 закрыт | Backend (commit `2e0bf8d`): миграция 27 `traffic_topup_options` (id/code/title/extra_traffic_gb/amount/is_enabled/sort_order/badge_label) + seed `topup50`/`topup100`/`topup200`. Модель `TrafficTopupOption` + `TrafficTopupOptionRepository`. `PricingService` рефактор на async API: `list_topups`/`get_topup`/`calculate_topup_basket(session, …)`; `TopUpOption` теперь содержит `sort_order`/`badge_label`/computed `price_per_gb`/`is_best_price`. `topup_keyboard(subscription_id, topups)` принимает список из БД. Best-price/ГБ автоматически бейджит опцию-победителя при ≥ 2 включённых и уникальном минимуме. Admin CRUD `/admin/upsells/traffic/` (commit `2f5f427`): list + create + per-row inline edit + toggle + delete, 4 новых `AuditAction` (created/updated/deleted/toggled). Promo-on-topup закрыт через существующий balance-flow + UX-hint в `render_invoice_text` для `InvoicePurpose.topup`. | (этот коммит) |
| 2026-05-09 | Sprint 2: FEA-NOTIF admin-UI | 3 коммита (commits `173584d`/`da13c86`/`f6f839a`): /admin/notifications/ + карточка с Prometheus-snapshot, edit + toggle + Jinja2-preview + JSON-валидация клавиатуры (4 AuditAction), test-send через NotificationDispatcher.dispatch(force=True) на первый admin_ids (force обходит is_enabled/snooze/cooldown). | (этот коммит) |
