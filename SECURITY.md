...
- [~] **SEC-H4.** Self-XSS / HTML-injection в админ-странице инвойса: HTML собирается f-строками, `html.escape` руками — паттерн хрупкий, отсутствует CSP.
  ...
  - Частично закрыто 2026-05-07: добавлены security-заголовки на все admin-ответы (CSP, X-Frame-Options: DENY, X-Content-Type-Options: nosniff, Referrer-Policy: same-origin, HSTS на HTTPS).
  - Дополнительно закрыто 2026-05-08: invoice-страницы давно были на Jinja (`admin_invoice_detail.html`, `admin_invoices.html`); удалён dead-code f-string `_invoice_detail_html`/`_invoice_list_html`/`_invoice_history_pretty_rows`/`_invoice_pretty_json` из `app/web/routes.py` (-130 строк). Inline `<style>`/`<script>` из `base.html` и `admin_broadcasts.html` вынесены в `/static/admin.css` + `/static/admin.js` + `/static/admin_broadcasts.js`; 8 inline `onsubmit="return confirm(...)"` заменены на `data-confirm` + delegated listener. CSP `script-src` сужен до `'self' https://cdn.tailwindcss.com` (без `'unsafe-inline'`).
  - Остаётся (low-prio): `style-src 'unsafe-inline'` всё ещё нужен — Tailwind Play CDN инжектит styles в runtime + по шаблонам/`routes.py` много inline `style="..."` attribute'ов. Полное removal требует build-step Tailwind в статический CSS-bundle и рефактор inline-style → utility-классы; отдельная задача.
