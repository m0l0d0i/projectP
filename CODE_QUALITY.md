...
- [x] **QC-7.2.** **N+1 в `app/services/notifications.py:377`** — `for ticket in tickets_to_close: user = await user_repo.get_by_id(ticket.user_id)`. Закрыто 2026-05-07: `due_auto_close` теперь делает `selectinload(SupportTicket.user)`, цикл читает уже загруженный `ticket.user` без отдельных запросов.
...
- [x] **OPS-2 (P1).** Rate-limit на `platega_callback` и Marzban-callback'и (Redis token bucket, у проекта уже есть Redis + `anti_spam.py` шаблон). Закрыто 2026-05-07/2026-05-08: `platega_callback` ограничен 60 req / 60s per IP через `cache.check_rate_limit` (commit `f8ea738`). Marzban-callback изначально не существует — Marzban вызывается только outbound через `MarzbanClient`/httpx; единственные inbound POST это `platega_callback` (rate-limited) и `telegram_webhook_path` (защищён `secret_token` от aiogram). Если когда-нибудь появится Marzban→bot интеграция, шаблон `_rate_limit_callback` уже готов в `webhooks.py`.
- [x] **OPS-3 (P1).** Circuit breaker над `MarzbanClient` и `PlategaProvider` — `tenacity` + лёгкий wrapper. Предотвращает каскадные отказы. Закрыто 2026-05-07: добавлен `app/services/circuit_breaker.py` (CLOSED → OPEN после 5 fails → HALF_OPEN после 30s → пробный вызов → CLOSED/OPEN). Marzban `_request` и Platega `_request` обёрнуты, 4xx и UserAlreadyExistsError не считаются failure.
- [~] **OPS-4 (P2).** **Outbox pattern** для transactional Telegram-сообщений: после оплаты записывать запись в `outbox` той же транзакцией; воркер доставляет exactly-once. Закрывает класс багов «Я заплатил, ничего не пришло». Базовая инфраструктура закрыта 2026-05-07: миграция `20260507_000024`, модель `OutboxMessage`, `OutboxRepository` (claim_due c FOR UPDATE SKIP LOCKED, exponential backoff, partial unique на correlation_key), `OutboxDispatcher` сервис, APScheduler job каждые 5с. Refactor `_consume_paid_invoice` → enqueue уведомления об оплате атомарно с `invoice.status = consumed`. **2026-05-08:** outbox расширен поддержкой `reply_markup` (`InlineKeyboardMarkup.model_dump`/`model_validate`) и `user_id` для propagation `bot_blocked` на TelegramForbiddenError; `notifications.py:check_expiring`/`check_low_traffic`/`check_traffic_exhaustion` переведены на outbox в той же транзакции, что флипает `notified_*` флаг → исчезла гонка «отправили, флаг не обновили» / «обновили флаг, не отправили». Остаются на outbox: `notifications.py:check_monthly_traffic_reset` + `check_support_ticket_auto_close`, broadcasts (`broadcast_polling.py`) — низкий приоритет.
